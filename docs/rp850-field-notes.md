...
Measured behavior of *this project's* Rongta RP850 unit — the stuff a
datasheet doesn't tell you, learned on paper. If you're building the project
...
flipping the slip — the leader becomes bottom margin), and the dashboard /
...
after a flashing session. Printing still works while mute; only probes die,
and the firmware refuses jobs without them (it can't verify paper).

## Memorize-then-print (GS * / GS /)

- Supported, **column-major data**, and the spec's ~12KB size cap is
  fiction on this unit: probed accepting **100 blocks = 800 rows ≈ 58KB**
...
Prints run darker than the dither — mildly in highlights, fatally in
