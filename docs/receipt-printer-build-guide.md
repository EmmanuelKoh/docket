...
- MAX3232 board (RS232-to-TTL) **with a male DB9 plug** — plugs straight into
...
Sketch — set the two pin numbers from the table:
```cpp
...
- **The TX/RX label trap (read this before wiring).** Serial convention says
  ...
  below *passes only when the module's VCC is disconnected* — the unpowered
  output stops fighting.)
- Plug the board's male DB9 into the printer's serial port.

Sketch — pins from your table row; baud is 115200 from Stage 0:
```cpp
...
The server already guarantees never-lose (atomic claim + lease); this stage
...
- **Wifi off mid-print** (kill the router/AP briefly) → the stream stalls,
  ...
  complete. A torn half-receipt in the bin is success — the full one
  follows.
...
The Mac is now out of the system: plugins fire, receipts print, laptop
closed.
...
| Loopback passes only with module VCC unplugged | Same — wrong TX/RX choice; swap the data wires |
