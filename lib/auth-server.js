...
async function init() {
  ...
  return betterAuth({
    ...
    // Dev only: allow off-device testing (phone over the LAN IP or a
    // cloudflared/ngrok tunnel) past Better Auth's CSRF origin check.
    // Scheme-less wildcards match on host, so they cover http and https.
    // Production pins BETTER_AUTH_URL and never gets these.
