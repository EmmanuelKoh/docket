...
// Earliest scheduled moment across the owner's enabled plugins, or null.
export async function earliestDueAt(ownerId) {
  const times = readStore()
    .filter(p => p.ownerId === ownerId && p.enabled && Number.isFinite(p.nextDueAt))
    .map(p => p.nextDueAt);
