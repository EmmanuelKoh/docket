// Templates: JSON-file driver (local dev fallback).
// Reads/writes data/templates.json, seeded from reference/starter-templates.json.
// Vercel's filesystem is read-only, so writes are disabled there with this
// driver — deploy with STORE_DRIVER=redis for a writable hosted studio.
...
function readStore() {
  ...
  return JSON.parse(fs.readFileSync(DATA_FILE, 'utf-8')).filter(mine);
}
...
export async function saveTemplate({ name, template, data }) {
  ...
  const entry = { name, template, data, ownerId: OWNER_ID, updatedAt: new Date().toISOString() };
  if (idx >= 0) {
    templates[idx] = entry;
  } else {
