...
// Earliest scheduled moment in the owner's due-index, or null. One
// command — this feeds the tick flag so idle ticks can skip Redis.
export async function earliestDueAt(ownerId) {
  const redis = await getRedis();
  const r = await redis.zrange(dueKey(ownerId), 0, 0, { withScores: true });
  return r.length >= 2 ? Number(r[1]) : null;
}
