...
export async function cancelJob(ownerId, id) {
  ...
  if (!row) return false;
  if (HOSTED) {
    const inPrinterHands = await redisAnd(async (redis, rkey) => {
      const removed = await redis.lrem(rkey(ownerId, 'jobs', 'queue'), 1, id);
      if (removed) return false;
      // Not in the queue. In flight = the printer may be receiving it —
      // refuse. In NEITHER structure = a zombie record (status says
      // queued but nothing can ever print it); cancel is exactly the
      // cleanup for that.
      const lease = await redis.zscore(rkey(ownerId, 'jobs', 'inflight'), id);
      return lease !== null;
    });
    if (inPrinterHands) return false;
...
export async function nackJob(ownerId, id) {
  ...
  if (HOSTED) {
    await redisAnd(async (redis, rkey) => {
      const held = await redis.zrem(rkey(ownerId, 'jobs', 'inflight'), id);
      if (held) {
        await redis.lpush(rkey(ownerId, 'jobs', 'queue'), id);
        return;
      }
      // No lease. Usually the expiry already requeued it — but a zombie
      // record (in neither structure) needs putting back for real, or
      // "Requeue" would only repaint the status.
      const pos = await redis.lpos(rkey(ownerId, 'jobs', 'queue'), id);
      if (pos === null) await redis.lpush(rkey(ownerId, 'jobs', 'queue'), id);
    });
